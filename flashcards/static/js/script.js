...
document.addEventListener("DOMContentLoaded", function() {
    ...
    console.log("Explanation? ", explanationTextarea);
    ...
    if (quillQuestionEditor && quillAdditionalEditor) {
        ...
        if (form && submitButton) {
            ...
            submitButton.addEventListener('click', function(e) {
                ...
                const additionalTextarea = document.getElementById('id_additional_content');
...
function showNextCard() {
    const cards = document.querySelectorAll('.flashcard');
    currentCardIndex = (currentCardIndex + 1) % cards.length;  // Wrap around to the first card
    window.location.hash = `#flashcard-${cards[currentCardIndex].id.split('-')[1]}`;
    // If on the /submit_answer path, redirect back to the root with the card hash
    if (window.location.pathname.startsWith('/submit_answer')) {
        window.location.href = `/#flashcard-${cards[currentCardIndex].id.split('-')[1]}`;
    } else {
...
function showPreviousCard() {
    const cards = document.querySelectorAll('.flashcard');
    currentCardIndex = (currentCardIndex - 1 + cards.length) % cards.length;  // Wrap around to the last card

    // If on the /submit_answer path, redirect back to the root with the card hash
    if (window.location.pathname.startsWith('/submit_answer')) {
        window.location.href = `/#flashcard-${cards[(currentCardIndex + 1) % cards.length].id.split('-')[1]}`;
    } else {
        showCard(currentCardIndex);
    }
    console.log("showPreviousCard")
}
...
function editCurrentCard() {
    const cards = document.querySelectorAll('.flashcard');
    console.log("Edit Current Card Index is now:", currentCardIndex);
    ...
    // Ensure currentCard has an ID
    if (!currentCard.id || !currentCard.id.includes('-')) {
        console.error("Invalid card ID format:", currentCard.id);
        return;
    }
    ...
    console.log("Navigating to edit card with ID:", cardId);

    // Redirect to the edit page with the card ID
